...
def _acquire_singleton_lock(lock_path) -> "tuple[Optional[object], str]":
    ...
    dispatcher: concurrent dispatchers double the reclaim frequency (each runs
    its own ``release_stale_claims`` -> promote -> dispatch loop), double
    ...
    Delegates to ``gateway.status._try_acquire_file_lock`` (``fcntl`` on POSIX,
    ``msvcrt`` on Windows) so the guard is cross-platform. Returns
    ``(handle, "held")`` on success (caller holds the handle for the process
    lifetime and must release via :func:`_release_singleton_lock`),
    ``(None, "contended")`` when another process holds it (caller must NOT
    dispatch), or ``(None, "unavailable")`` when locking can't be performed
    (caller falls back to config-only control).
    """
    try:
...
class GatewayKanbanWatchersMixin:
    ...
    async def _kanban_dispatcher_watcher(self) -> None:
        ...
        # Single-dispatcher backstop (upstream port 2026-06-25). dispatch_in_gateway
        # defaults to true, so a new-profile gateway (or a same-profile restart
        # race) can silently start a second dispatcher; concurrent dispatchers
        # double reclaim frequency, double claim-attempt events, and -- with
        # wal_autocheckpoint=0 -- concurrent manual WAL checkpoints can corrupt
        # index pages. The lock lives at the machine-global kanban root (shared
        # across profiles by design), so it serialises ALL gateways. Falls back to
        # config-only control when advisory locking is unavailable.
        ...
        try:
            while self._running:
                ...
                try:
                    ...
                    raise  # lock released by the finally below
                except Exception:
            ...
            # Release the singleton dispatcher lock on EVERY exit path —
            # normal loop exit, CancelledError, or any unexpected exception —
            # so a crashed dispatcher never strands the lock and blocks other
            # gateways from dispatching (upstream port hardening, run 2026-06-25).
