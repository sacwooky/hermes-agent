...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    ...
    if _trunk_remote is None:
        ...
        sys.exit(1)
    # Upstream (merged 2026-06-25): installer checkouts are shallow
    # (`git clone --depth 1`). A plain `git fetch` would unshallow the repo
    # (dragging in the whole history the shallow clone avoided) and the rev-list
    # count below would then report a bogus "behind" number. Detect shallow up
    # front and fetch with --depth 1 to preserve the boundary.
    is_shallow = (
        subprocess.run(
    ...
    fetch_result = subprocess.run(
        git_cmd + ["fetch"] + depth_args + [_trunk_remote, _trunk_branch],
        cwd=PROJECT_ROOT,
        capture_output=True,
        text=True,
    )
