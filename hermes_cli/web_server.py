...
def _dashboard_allowed_host_aliases() -> set[str]:
    raw = os.getenv("HERMES_DASHBOARD_ALLOWED_HOSTS", "")
    return {_host_without_port(item) for item in raw.split(",") if item.strip()}


def _is_accepted_host(host_header: str, bound_host: str) -> bool:
    """True if the Host header targets the bound interface or an operator alias.

    Accept exact bound host (or any host for 0.0.0.0/::), with special
    loopback handling so 127.0.0.1 and localhost remain interchangeable.
    Port suffixes are ignored. Explicit aliases are opt-in for trusted names
    like Tailscale MagicDNS hostnames; arbitrary DNS names stay rejected as
